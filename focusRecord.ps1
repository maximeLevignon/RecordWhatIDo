...
while($recording){
  ...
  ((Get-Date -Format FileDateTime) + '-' + ((Get-Process | Where-Object { $_.mainWindowHandle -eq [UserWindow]::GetForegroundWindow() }).Name) + '-[TopLeft{' + $rectangle.Left + ';' + $rectangle.Top + '}BottomRight{' + $rectangle.Right + ';' + $rectangle.Bottom) + '}]'
